...
<div id="main">
    ...
    <div id='bread'><a href='/'><?php echo __("Home")?></a> > <?php echo __("Users")?></div>
    <div class='page_header'><?php echo __("Users")?></div>
    ...
    <div class="col-lg-12 col-md-12">
		...
		<div class="form-group"><label for="k_user"><?php echo __("Find user")?></label>
			<input type="text" class="form-control" name="k_user" id="k_user" placeholder="<?php echo __("name")." ".__("or")." ".__("email")?>" required="required" value="<?php echo $k_search_user ?>">
		</div>
		<button type="submit" class="btn btn-info" title="<?php echo __("search")?>"><i class="fa fa-search"></i></button>
		<?php if ( $k_search_user!='' ) { ?><a href="./users.php" class="btn btn-danger" title="<?php echo __("clear")?>"><i class="fa fa-times-circle"></i></a><?php }?>
		</form>
    </div>
	...
	<table class="table table-condensed">
	      <thead>
	        <tr class="valign-middle">
	          ...
	          <th class="tocenter"><?php echo __("Profile")?></th>
	          <th class="tocenter"><?php echo __("Active")?></th>
		...
		if (count($arr_users)>0){
		 ...
		 ?>
		   <tr class="valign-middle">
	          ...
	          <td class="tocenter">
	          	  <?php $btn_active_css = ($item['activated']==1) ? "label-success" : "label-danger" ; $btn_active_txt = ($item['activated']==1) ? __("YES") : __("NO"); ?>
				  <form action="#" method="post">
		          <input type="hidden" name="k_user" value="<?php echo $k_search_user?>">
	          ...
	          <td class="tocenter">
		         ...
		         <?php } else { ?>
		          ...
		          <input type="hidden" name="user_id" value="<?php echo get_crypted_id($item["id_user"]);?>">
		          <button type="submit" class="btn btn-danger btn-sm"><?php echo __("Delete")?> <i class="fa fa-trash"></i></button>
		          </form>
		         <?php }  ?>
	          </td>	          
		...
		<?php } } else { ?>
			<tr><td colspan="7" class="alert alert-danger"><?php echo __("Nothing found")?></td></tr>
		<?php } ?>
		</tbody>
	</table>
	<!-- /contents -->
