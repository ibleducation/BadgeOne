...
<div id="main">
    ...
    <div id='bread'><a href='/'><?php echo __("Home")?></a> > <?php echo __("Issuer Panel")?></div>
    <div class='page_header'><?php echo __("Issuer : Managed badges")?> <br></div>
    ...
    <div class="">
    	...
    	<div class="pull-left">
    		<form action="#" method="post" class="form-inline">
				<div class="form-group"><label for="k_user"><?php echo __("Select Issuer")?></label>
				...
				<button type="submit" class="btn btn-info" title="<?php echo __("search")?>"><i class="fa fa-search"></i></button>
				<?php if ( $k_search_issuer!='' ) { ?><a href="./issuer.php" class="btn btn-danger" title="<?php echo __("clear")?>"><i class="fa fa-times-circle"></i></a><?php }?>
			</form>
    	</div>
    	<?php }   ?>
    	<div class="pull-right">
    		<form action="badge_edit.php" method="post"><button type="submit" class="btn btn-success btn-md"> <?php echo __("ADD NEW BADGE")?> <i class="fa fa-star"></i></button></form>
    	</div>
    </div>
	...
	<table class="table table-condensed">
	      <thead>
	        <tr class="valign-middle">
	          ...
	          <th class="tocenter"><?php echo __("Earn")?></th>
	          <th class="tocenter"><?php echo __("Revoke")?></th>
	          <th class="tocenter"><?php echo __("Delete")?></th>
	        </tr>
	      </thead>
	      <tbody>
		<?php 
		...
		if (count($arr_badges_list)>0) {
		 ...
		 ?>
		   <tr class="valign-middle">
	          ...
	          <td class="tocenter"><?php echo ($item['enabled']==1) ? "<span class=\"label label-success\">".__("YES")."</span>" : "<span class=\"label label-danger\">".__("NO")."</span>"?></td>
	          
	          <td class="tocenter">
	          	  <?php $btn_publish_css = ($item['published']==1) ? "label-success" : "label-danger" ; $btn_publish_txt = ($item['published']==1) ? __("YES") : __("NO"); ?>
	          ...
	          <td class="tocenter">
				 <?php if ( $total_b_count > 0 ) { ?>
	         		<a href="#" class="btn btn-default clearmargin" onclick="alert('<?php echo __("Could not be modified")?>');return false;"><?php echo __("Edit")?> <i class="fa fa-edit"></i></a>
		          <?php } else { ?>
		          <form action="badge_edit.php" method="post" class="clearmargin"><input type="hidden" name="badge_id" value="<?php echo get_crypted_id($item["badge_id"]);?>"><button type="submit" class="btn btn-primary btn-sm clearmargin"> <?php echo __("Edit")?> <i class="fa fa-edit"></i></button></form>
		          <?php }  ?>
			  </td>	
	          ...
	          <td class="tocenter">
	          	  <?php if ($item['enabled']==1) { ?>
			          ...
			          <button type="submit" class="btn btn-info btn-sm" type="button"> <?php echo __("Earn")?> <span class="badge"><?php echo $total_b_earns?></span></button>
			          </form>
		          <?php } else {  ?>
		          	<a href="#" class="btn btn-default btn-sm clearmargin" onclick="alert('<?php echo __("Badge not enabled")?>');return false;"><?php echo __("Earn")?> <span class="badge"><?php echo $total_b_earns?></span></a>
		          <?php }   ?>
	          </td>
	          ...
	          <td class="tocenter">
	          	<?php if ($total_b_revok>0) {  ?>
			    	<form action="badge_revoked.php" method="post">
			        	<input type="hidden" name="badge_id" id="badge_id" value="<?php echo get_crypted_id($item["badge_id"]);?>">
						<button type="submit" class="btn btn-warning btn-sm" type="button"> <?php echo __("Revk")?> <span class="badge"><?php echo $total_b_revok?></span></button>
					</form>  
				<?php } else { ?>
					<a href="#" class="btn btn-default btn-sm clearmargin" onclick="alert('No data');return false;"><?php echo __("Revk")?> <span class="badge"><?php echo $total_b_revok?></span></a>
				<?php }?>
	          </td>
	          
	          <td class="tocenter">
	          	 <?php if ( $total_b_count > 0 ) { ?>
	          	  <a href="#" class="btn btn-default btn-sm" onclick="alert('<?php echo __("Could not be deleted")?>');return false;"><?php echo __("Delete")?> <i class="fa fa-trash"></i></a>
		         <?php } else { ?>
		...
		<?php } } else { ?>
			<tr><td colspan="9" class="alert alert-danger"><?php echo __("Nothing found")?></td></tr>
		<?php } ?>
...
<div class="container"><div class="col-lg-12 alert alert-info">
	<blockquote>
		<li><strong><?php echo __("Enabled")?></strong>: <?php echo __("The object contains all needed information to be published")?></li>
		<li><strong><?php echo __("Published")?></strong>: <?php echo __("The object is released to be earned")?></li>
	</blockquote>
	<p><?php echo __("If you want to disable temporally or permanently the earn process, just unpublish the item.")?></p>
</div></div>
